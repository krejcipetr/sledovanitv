...
RUN /bin/test -d /usr/local/sledovanitv ] || mkdir /usr/local/sledovanitv
RUN /bin/test -d /recordings ] || mkdir /recordings
...
RUN /bin/test -d /config/sledovanitv ] || mkdir /config/sledovanitv
